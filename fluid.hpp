...
static inline float bswap_float(float x) {
    ...
    __x.f = x;
    __x.i = ((__x.i & 0xff000000) >> 24) | ((__x.i & 0x00ff0000) >> 8) | ((__x.i & 0x0000ff00) << 8) |
            ((__x.i & 0x000000ff) << 24);
...
class Vec3 {
   ...
   public:
    ...
    Vec3() {
        data[0] = _mm_set1_pd(0.f);
        data[1] = _mm_set1_pd(0.f);
    }
...
#pragma warning(disable : 1684)  // warning #1684: conversion from pointer to same-sized integral type (potential
                                 // portability problem)
